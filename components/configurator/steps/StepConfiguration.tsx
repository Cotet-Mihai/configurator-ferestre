  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-xl mx-auto px-4">
      ...
      <div className="flex flex-col gap-5">
        ...
        {/* Culoare — carduri vizuale cu mostră + modificator preț */}
        {product.colorOptions.length > 0 && (
          <div className="flex flex-col gap-2">
            <span className="text-sm font-medium text-zinc-700">Culoare</span>
            <div className="flex flex-wrap gap-2">
              {product.colorOptions.map((o) => {
                const selected = state.selectedColor === o.id;
                return (
                  <button
                    key={o.id}
                    type="button"
                    onClick={() => dispatch({ type: 'SET_COLOR', payload: o.id })}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-all duration-150
                      ${selected
                        ? 'border-amber-600 bg-amber-50 text-zinc-900 shadow-sm'
                        : 'border-zinc-200 bg-white text-zinc-700 hover:border-zinc-400'
                      }`}
                  >
                    <span
                      className="w-5 h-5 rounded-full border border-black/10 flex-shrink-0"
                      style={{ backgroundColor: o.colorValue ?? '#ccc' }}
                    />
                    <span>{o.label}</span>
                    <span className={`text-xs font-medium ${o.priceModifier ? 'text-amber-700' : 'text-zinc-400'}`}>
                      {o.priceModifier ? `+${o.priceModifier} lei/m²` : 'Inclus'}
                    </span>
                  </button>
                );
              })}
